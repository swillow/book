subroutine pot_nasa(r1, e1, dr1)
  ...
  use nasa_mod
  implicit none
  real(c_double), dimension(9), intent(in)  :: r1
  real(c_double), dimension(9), intent(out) :: dr1
  real(c_double), intent(out) :: e1
  ...
  integer          :: j, inI, inJ, inK, ix
  double precision :: deoh, phh1
  double precision, dimension(245) :: c5z

  do ix = 1, 3
     ROH1(ix) = r1(3+ix) - r1(ix) ! H1 - O
     ROH2(ix) = r1(6+ix) - r1(ix) ! H2 - O
     RHH(ix)  = r1(3+ix) - r1(6+ix) ! H1 - H2
  end do
  ...
  do ix = 1, 3
     dr1(3+ix) = dVa1*ROH1(ix) + dVb*RHH(ix) + dVcdr1*ROH1(ix) + &
          dVcdcth*(ROH2(ix)/(dROH1*dROH2)-costh*ROH1(ix)/(dROH1*dROH1))
     dr1(6+ix) = dVa2*ROH2(ix) - dVb*RHH(ix) + dVcdr2*ROH2(ix) + &
          dVcdcth*(ROH1(ix)/(dROH1*dROH2)-costh*ROH2(ix)/(dROH2*dROH2))
     dr1(0+ix) = -(dr1(3+ix)+dr1(6+ix))
  end do
  
  dr1 = dr1*.00285914375100642899d0 ! cm-1 --> kcal/mol
